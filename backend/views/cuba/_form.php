...
<div class="cuba-form">
    ...
    <div class="row">
        ...
        <div class="col-lg-2">
            <?= $form->field($modelCuba, 'peso_bruto', [
                'inputTemplate' => '<div class="input-group">{input}<span class="input-group-addon">Kg</span></div>',
                'inputOptions' => [
                    'value' => yii::$app->formatter->asInteger($modelCuba->peso_bruto),
                    'style' => 'text-align: right',
                ]
            ])?>
        </div>
        <div class="col-lg-2">
            <?= $form->field($modelCuba, 'tara', [
                'inputTemplate' => '<div class="input-group">{input}<span class="input-group-addon">Kg</span></div>',
                'inputOptions' => [
                    'value' => yii::$app->formatter->asInteger($modelCuba->tara),
                    'style' => 'text-align: right',
                ]
            ])?>
        </div>
                ...
                <?php foreach ($modelsCompartimento as $i => $modelCompartimento): ?>
                    <div class="item "><!-- widgetItem -->
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-3">
                                    <?= $form->field($modelCompartimento, "[{$i}]capacidad", [
                                        'inputTemplate' => '<div class="input-group">{input}<span class="input-group-addon">litros</span></div>',
                                        'inputOptions' => [
                                            'value' => yii::$app->formatter->asInteger($modelCompartimento->capacidad),
                                            'style' => 'text-align: right',
                                        ]
                                    ])?>
                                </div>
